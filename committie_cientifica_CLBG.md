---
layout: committees
title: Comissão Científica do 10CLBG
group: Comissões-10CLBG

committees:

  - role: Membros
    people:
      - name: André Estevão Ferreira da Silva
        affiliation: HUESKER
      - name: António Campos e Matos
        affiliation: GEG
      - name: Antônio Thomé
        affiliation: UPF        
      - name: António Silva Cardoso
        affiliation: UP
      - name: António Viana da Fonseca
        affiliation: UP
      - name: Carlos Medeiros Silva
        affiliation: EMBRE        
      - name: Carlos Henrique Medeiros
        affiliation: 
      - name: Cláudio Mahler
        affiliation: UFRJ   
      - name: Jaime Santos
        affiliation: UL
      - name: João Bilé Serra
        affiliation: LNEC
      - name: Jorge Almeida e Sousa
        affiliation: UC
      - name: José Mateus de Brito
        affiliation: 
      - name: José Vieira Lemos
        affiliation: LNEC
      - name: Leonardo de Bona Becker
        affiliation: UFRJ      
      - name: Luís Lamas
        affiliation: LNEC
      - name: Luiz Carlos de Figueiredo
        affiliation: 
      - name: Luiz Guilherme Rodrigues de Mello
        affiliation:               
      - name: Nadir Plasência
        affiliation: EDP
      - name: Neusa Maria Bezerra Mota
        affiliation: BMS Engenharia
      - name: Tácio Mauro Pereira de Campos
        affiliation: PUC-Rio        
---